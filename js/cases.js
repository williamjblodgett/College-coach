...
(function () {
  ...
  function schedule(state,c,weeks){var due=(state.season.week||1)+(weeks||1);c.nextYear=state.career.year;c.nextWeek=due;if(due>(state.season.totalRegWeeks||14)){c.nextYear++;c.nextWeek=1;}ensure(state).pendingCaseDecision=null;}
  ...
  var Cases={
    ...
    advanceWeek:function(state){var i=ensure(state);if(i.pendingCaseDecision)return i.pendingCaseDecision;i.mediaPressure=clamp(i.mediaPressure-1,0,100);for(var n=0;n<i.openCases.length;n++){var c=i.openCases[n];c.timeline=c.timeline||[];c.deadlineYear=c.deadlineYear||c.openedYear+2;if(state.career.year>c.deadlineYear&&c.stage!=='hearing'&&c.stage!=='appeal'){c.stage='hearing';c.evidence=clamp(c.evidence+1,0,9);c.timeline.push({year:state.career.year,week:state.season.week,event:'Deadline forced a hearing'});}if(state.career.year>(c.nextYear||c.openedYear)||(state.career.year===(c.nextYear||c.openedYear)&&(c.nextWeek||0)<=state.season.week)){i.pendingCaseDecision=decision(c);return i.pendingCaseDecision;}}return null;},
    resolve:function(state,optionId){var i=ensure(state),p=i.pendingCaseDecision,c=p&&active(state,p.caseId);if(!p||!c)return null;var choice={year:state.career.year,week:state.season.week,phase:p.phase,choice:optionId};c.choices.push(choice);c.timeline=c.timeline||[];c.timeline.push({year:state.career.year,week:state.season.week,event:p.phase+': '+optionId});
